from tkinter import *
import math
# ---------------------------- CONSTANTS ------------------------------- #
PINK = "#e2979c"
RED = "#e7305b"
GREEN = "#9bdeac"
YELLOW = "#f7f5dd"
FONT_NAME = "Courier"
WORK_MIN = 25
SHORT_BREAK_MIN = 5
LONG_BREAK_MIN = 20
reps = 0
timer = None

# ---------------------------- TIMER RESET ------------------------------- # 

def reset_timer():
    window.after_cancel(timer)
    canvas.itemconfig(timer_text, text='00:00')
    timer_label.config(text='Timer')
    check_label.config(text='')
    global reps
    reps = 0

# ---------------------------- TIMER MECHANISM ------------------------------- # 

def start_timer():
    global reps
    reps += 1
    work_sec = WORK_MIN * 60
    short_break_sec = SHORT_BREAK_MIN
    long_break_sec = LONG_BREAK_MIN


    # If it's the 8th rep:
    if reps % 8 == 0:
        count_down(long_break_sec)
        timer_label.config(text='BREAK', fg=RED)

    # If it's the 2nd/4th/6th rep:
    if reps % 2 == 0:
        count_down(short_break_sec)
        timer_label.config(text='BREAK', fg=PINK)

    else:
        count_down(work_sec) # If it's the 1st/3rd/5th/7th rep:
        timer_label.config(text='WORK', fg=GREEN)

# ---------------------------- COUNTDOWN MECHANISM ------------------------------- # 
def count_down(count):

    count_min = math.floor(count/60) # minutes
    count_sec = count % 60 # seconds that remain
    if count_sec < 10:
        count_sec = f'0{count_sec}'
    canvas.itemconfig(timer_text, text=f'{count_min}:{count_sec}')
    if count > 0:
        global timer
        timer = window.after(1000, count_down, count-1)

    else:
        start_timer()
        mark = ''
        for _ in range(math.floor(reps/2)): # total of work sessions
            mark += '✔'

        check_label.config(text=mark)
    return count


# ---------------------------- UI SETUP ------------------------------- #

window = Tk()
window.title('Pomodoro')
window.config(padx= 100, pady=50, bg=YELLOW)

canvas = Canvas(width=200, height=224, bg=YELLOW, highlightthickness=0)
tomato_img = PhotoImage(file='tomato.png')
canvas.create_image(100, 112, image=tomato_img)
timer_text = canvas.create_text(100, 130, text='00:00', fill='white', font=(FONT_NAME, 30, 'bold'))
canvas.grid(column= 1, row= 1)

timer_label = Label(text='Timer', font=(FONT_NAME, 30, 'bold'), fg=GREEN, bg=YELLOW)
timer_label.grid(column=1, row=0) # to show on the screen
timer_label.config(padx= 10, pady= 10)

check_label = Label(font=(FONT_NAME, 15, 'bold'), fg=GREEN, bg=YELLOW)
check_label.grid(column=1, row=3) # to show on the screen
check_label.config(padx= 2, pady= 2)

start_button= Button(text= 'Start', command=start_timer, font=(FONT_NAME, 15, 'bold'))
start_button.grid(column=0, row=2)
start_button.config(padx= 2, pady= 2)

reset_button= Button(text= 'Reset', font=(FONT_NAME, 15, 'bold'), command=reset_timer)
reset_button.grid(column=2, row=2)
reset_button.config(padx= 2, pady= 2)

window.mainloop()